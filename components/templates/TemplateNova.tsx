...
export default function TemplateNova({ data }: { data: ResumeData }) {
  ...
  const SectionHeading = ({ label }: { label: string }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
      <div style={{ width: '3px', height: '16px', backgroundColor: custom.primaryColor || '#059669', borderRadius: '2px' }} />
      <h2 style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.12em', color: custom.headingColor || '#065f46' }}>
        {label}
      </h2>
      <div style={{ flex: 1, height: '1px', backgroundColor: `${custom.primaryColor || '#059669'}30` }} />
    </div>
  );
              ...
              {data.experience.map((exp, i) => (
                <div key={exp.id} className="resume-entry">
                  <div className="flex justify-between items-baseline mb-0.5">
                    <h3 style={{ fontSize: '10.5px', fontWeight: 700, color: custom.headingColor || '#065f46' }}>
                      <EditableText path={`experience.${i}.role`} value={exp.role} />
                    </h3>
                    <span style={{ fontSize: '9px', color: custom.primaryColor || '#059669', fontWeight: 600, backgroundColor: `${custom.primaryColor}15`, padding: '1px 6px', borderRadius: '10px' }}>
                  ...
                  <div style={{ fontSize: '9.5px', fontWeight: 600, color: custom.primaryColor || '#059669', marginBottom: '3px' }}>
                    <EditableText path={`experience.${i}.company`} value={exp.company} />
                    {exp.location && <span style={{ fontWeight: 400, color: custom.textColor, opacity: 0.7 }}> · <EditableText path={`experience.${i}.location`} value={exp.location} /></span>}
                  </div>
              ...
              {data.education.map((edu, i) => (
                <div key={edu.id} className="flex justify-between items-start resume-entry">
                  <div>
                    ...
                    <div style={{ fontSize: '9.5px', color: custom.primaryColor || '#059669', fontWeight: 500 }}>
                  ...
                  <span style={{ fontSize: '9px', color: custom.primaryColor, fontWeight: 600, backgroundColor: `${custom.primaryColor}15`, padding: '1px 6px', borderRadius: '10px', whiteSpace: 'nowrap' as const }}>
      ...
      case 'skills':
        ...
        return (
          <div key={id} className="mb-4 resume-section">
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '4px 6px', fontSize: '10px' }}>
              {data.skills.map((skill, i) => (
                <div key={i} className="flex items-center">
                  <EditableText path={`skills.${i}`} value={skill} />
                  {i < data.skills.length - 1 && <span style={{ color: custom.primaryColor, marginLeft: '2px' }}>·</span>}
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div style={{ width: "100%", minHeight: "100%", backgroundColor: custom.bgColor || "#ffffff", fontFamily: `${custom.fontFamily || 'Inter'}, sans-serif`, color: custom.textColor || '#1f2937', position: 'relative' }}>
      {/* Nova header: left-aligned with thick colored left border */}
      <div style={{ padding: '20px 32px', borderLeft: `6px solid ${custom.primaryColor || '#059669'}`, marginLeft: '24px', marginTop: '20px', marginBottom: '16px' }}>
        {data.personal.photo && (
          <img src={data.personal.photo} alt="Profile"
            style={{ width: 56, height: 56, borderRadius: '8px', objectFit: 'cover', border: `2px solid ${custom.primaryColor}`, float: 'right' }} />
        )}
        <h1 style={{ fontSize: '28px', fontWeight: 800, color: custom.headingColor || '#065f46', letterSpacing: '-0.02em', marginBottom: '2px' }}>
          <EditableText path="personal.fullName" value={data.personal.fullName} />
        </h1>
        {data.personal.title && (
          <p style={{ fontSize: '11px', color: custom.primaryColor || '#059669', fontWeight: 600, marginBottom: '12px', letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>
            <EditableText path="personal.title" value={data.personal.title} />
          </p>
        )}
        <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '4px 14px', fontSize: '9.5px', color: custom.textColor || '#374151' }}>
